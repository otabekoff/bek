<template lang="pug">
bek-inputbox.bek-datepicker(:invalid="!bekValidate",
  ...
  .bek-datepicker-box(slot="out")
    .header
      .bek-input-group.header-buttons
        bek-button.bek-size-sm(@click.prevent="moveValue('year', -1)", tabindex="-1")
          .icon(:style="{ transform: icons.prevYear }", v-html="icons.iconY")
        bek-button.bek-size-sm(@click.prevent="moveValue('month', -1)", tabindex="-1")
          .icon(:style="{ transform: icons.prevMonth }", v-html="icons.icon")
      .bek-grow.bek-text-center
...
export default {
  ...
  methods: {
    ...
    checkBekValidity(day, month, year) {
      if (typeof this.required === 'function') {
    ...
    isDateDisabled(day, month, year) {
      return !this.checkBekValidity(day, month, year);
    },
  },
};
</script>
